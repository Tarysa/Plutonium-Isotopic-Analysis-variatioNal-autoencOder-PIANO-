...
class CLRLoss(nn.Module):
    ...
    def _clr_transform(self, x):
        ...
        Args:
            x: Tensor of shape [batch, num_components]
               Must be positive and sum to 1 (or close to 1)
        
        Returns:
            CLR-transformed tensor of same shape
        """
        # Add epsilon to avoid log(0)
        x_safe = x + self.eps
        
        # Compute geometric mean: (x1 * x2 * ... * xn)^(1/n)
        # Using log for numerical stability: exp(mean(log(x)))
        log_x = torch.log(x_safe)
        geom_mean = torch.exp(torch.mean(log_x, dim=-1, keepdim=True))
        
        # CLR: log(x_i / geometric_mean)
        clr = torch.log(x_safe / geom_mean)
...
class VAE(nn.Module):
    ...
    """
    def __init__(self, config, device, input_dim, input_class_dim=3, num_outputs=5):
        super().__init__()
        ...
        self.regressor = Regressor(config, num_dense_layers= config.nb_dense_layer, use_physical_data=False, 
                 input_class_dim=input_class_dim, num_classes=num_outputs, use_softmax=self.use_softmax)
    ...
    def forward(self, x, condition=None):
        ...
        if self.train_with_condition:
            ...
            reg_input = mu.clone()
        # Regression    
        normalized_preds = self.regressor(reg_input)

        if not(self.use_softmax):
            sum_preds = torch.sum(normalized_preds, dim=1, keepdim=True)
            normalized_preds = normalized_preds / sum_preds

        # Decode from latent space
        reconstruction = self.decoder(z, condition)[:, 0]
        
        # Return appropriate outputs based on configuration
        return (reconstruction, mu, log_sigma_2), normalized_preds
    ...
    def exact_reconstruction(self, x, condition=None):
        """
        ...
        # sum_preds = torch.sum(regression_pred, dim=1, keepdim=True)
        # normalized_preds = regression_pred / sum_preds
