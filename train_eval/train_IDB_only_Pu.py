...
sys.path.append('../')
from utils.preprocessing_IDB_only_Pu import *
from models.model_IDB_only_Pu import *
...
def setup_argument_parser():
    ...
    parser.add_argument('--only_Pu', type=bool, default=False)
    parser.add_argument('--only_U', type=bool, default=False)
...
def setup_datasets(config):
    ...
    if config.only_Pu:
        train_dataset = Dataset_dataset('../datasets/IDB/only_Pu2/train.json', config)
        val_dataset = Dataset_dataset('../datasets/IDB/only_Pu2/val.json', config)
    elif config.only_U:
        train_dataset = Dataset_dataset('../datasets/IDB/only_U/train.json', config)
        val_dataset = Dataset_dataset('../datasets/IDB/only_U/val.json', config)
    else:
...
def train_epoch(model, train_loader, optimizer, vae_loss, regression_loss, config, epoch, device):
    ...
    for batch, inputs in enumerate(train_loader):
        ...
        X,  pu_proportion, condition = inputs
        X,   pu_proportion, condition = X.to(device), pu_proportion.to(device), condition.to(device) 
        
        # Zero gradients
        optimizer.zero_grad()
        
        # Forward pass
        output_VAE, pred_norm_pu = model(X, condition)
        regloss = regression_loss(pred_norm_pu, pu_proportion.float())
