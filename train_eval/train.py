...
def setup_argument_parser():
    ...
    parser.add_argument('--use_softmax', type=bool, default=False, help='Use Softmax as last activation fonction')
    ...
    parser.add_argument('--preprocessing_version', type=int, default=1, help='Preprocessing version')
...
def train_epoch(model, train_loader, optimizer, vae_loss, regression_loss, config, epoch, device):
    ...
    for batch, inputs in enumerate(train_loader):
        ...
        # Forward pass
        output_VAE, pred_norm = model(X,condition= condition)
        regloss = regression_loss(pred_norm, y.float())
